"""History tab: recent jobs from the shared bucket (both Spaces).

Downloads go through the expiry-checked API routes; expired jobs are hidden
by the storage layer and their links return 410.
"""
...
from gpu.ui.common import download_url
...
def history_tab():
    gr.Markdown("Jobs from this Space and the CPU Toolbox. Outputs are kept for 24 hours.")
    ...
    def _select(prefixes, evt: gr.SelectData):
        ...
        try:
            ...
            return gr.update(visible=False), "", ""
        links = "".join(
            f"<li><a href='{download_url(prefix, o.id)}' download>{html.escape(o.filename)}</a>"
            f" <span class='dim'>({format_size(o.size)})</span></li>"
            for o in manifest.outputs
        )
        detail = (
            f"**{manifest.operation.replace('_', ' ').title()}** — `{html.escape(manifest.original_filename)}`  \n"
        ...
        return gr.update(visible=True), detail, prefix

    table.select(fn=_select, inputs=[prefixes_state],
                 outputs=[detail_group, detail_md, selected_prefix])

    def _delete(prefix):
        if prefix:
            get_services().storage.delete_job(prefix)
        rows, prefixes = _load_jobs()
        return rows, prefixes, gr.update(visible=False), "", ""

    delete_btn.click(fn=_delete, inputs=[selected_prefix],
                     outputs=[table, prefixes_state, detail_group, detail_md, selected_prefix])
